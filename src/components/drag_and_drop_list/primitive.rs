...
impl DragAndDropContext {
    ...
    fn drop(&mut self) {
        ...
        let mut keys = (self.item_keys)();
        let key = keys.remove(from);
        keys.insert(index, key);
        self.item_keys.set(keys);
    }
...
/// Returns the signal tracking the current item order (by the stable keys
/// passed in via [`DragAndDropListProps::item_keys`]), live-updated on every
/// drag/drop. Must be called from within a [`DragAndDropList`]'s `children`
/// — not available outside of that subtree. Returning the signal itself
/// (rather than a snapshot `Vec`) lets callers read it reactively from
/// inside their own `use_effect`, e.g. to mirror the order out to a signal
/// owned by an ancestor component outside the list's context subtree.
pub fn use_drag_and_drop_list_order() -> Signal<Vec<String>> {
...
pub struct DragAndDropListProps {
    ...
    /// Stable identifier for each entry in `items`, same length and order.
    /// Reordering permutes this in lockstep so [`use_drag_and_drop_list_order`]
    /// can report the final order without relying on `Element` identity.
...
pub fn DragAndDropList(props: DragAndDropListProps) -> Element {
    ...
    let item_keys = use_signal(|| props.item_keys.clone());
    ...
    rsx! {
        div { class: "atk-dnd-list", ..props.attributes,
            div {
                id: "dnd-instructions",
...
pub fn DragAndDropListItem(props: DragAndDropListItemProps) -> Element {
    ...
    rsx! {
        if (ctx.drop_position)() == DropPosition::Before
            && render_drop_indicator((ctx.drop_to)())
        {
            DropIndicator {}
        }
        li {
        ...
        if (ctx.drop_position)() == DropPosition::After
            && render_drop_indicator((ctx.drop_to)())
        {
            DropIndicator {}
        }
...
fn DropIndicator() -> Element {
    rsx! {
        div { class: "atk-dnd-drop-indicator" }
    }
}
