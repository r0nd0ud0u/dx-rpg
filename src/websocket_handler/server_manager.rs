...
use crate::core_game_data::CoreGameData;
...
impl ServerManager {
    ...
    pub fn add_server_data(&mut self, server_name: &str, core_game_data: &CoreGameData, player_name: &str) {
    ...
    pub fn add_player_to_server(&mut self, server_name: &str, player_name: &str, player_id: u32) {
        if let Some(server_data) = self.servers_data.get_mut(server_name) {
            ...
            if server_data.core_game_data.game_phase == GamePhase::Loading
                && let Some(character_name) = server_data.core_game_data.heroes_chosen.get(player_name)
            {
                server_data
                    .players_data
                    .players_info
